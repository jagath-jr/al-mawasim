...
export default function ContactClient({ initialData, siteSettings }: ContactClientProps) {
  
  // Extract Page Config Data
  const contactConfig = initialData?.contactConfig || {};
  
  // Fallbacks for intro text
  const defaultIntro = "AL MAWASIM DECOR & CURTAINS provides custom curtains, roller blinds, zebra blinds, SPC flooring, wallpaper installation, and upholstery solutions for residential, commercial, hospitality, and office spaces across Abu Dhabi. Our experienced team delivers premium materials, elegant designs, and professional installation services tailored to transform every space with style, comfort, and functionality.";
  const pageIntroduction = contactConfig.pageIntroduction || defaultIntro;

  // Extract Global Site Settings
  const phones = siteSettings?.phones?.map((p: any) => p.number) || ["+971 56 677 3793", "+971 55 521 8804"];
  const emails = siteSettings?.emails?.map((e: any) => e.email) || ["info@almawasimdecor.com"];
  const address = siteSettings?.address || {
    street: "Al Mawasim Decor",
      ...
      <section className="relative w-full h-[350px] md:h-[450px] flex items-center pt-10">
        ...
        >
          ...
          <div className="space-y-4">
            ...
            >
              {initialData?.title || "Get In Touch"}
            </motion.h1>
          </div>
      ...
      <div className="relative z-20 bg-white rounded-t-[2.5rem] shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-24 grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-16 overflow-hidden">
          ...
          >
            ...
            <div className="text-slate-600 mb-10 text-sm md:text-base leading-relaxed">
              {/* Rich text renderer goes here eventually. Standard string fallback for now. */}
              {typeof pageIntroduction === "string" ? pageIntroduction : defaultIntro}
            </div>
