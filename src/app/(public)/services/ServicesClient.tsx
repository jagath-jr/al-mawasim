...
// ========================================
// 1. DATA MODELS & FALLBACK CONTENT
// ========================================
...
const fallbackServicesData: ServiceSection[] = [
  {
    id: "curtains",
    title: "Curtains",
    subtitle: (
  ...
  },
  {
    ...
    description: "We provide sofa upholstery and custom furniture solutions using premium materials and expert craftsmanship. Our upholstery services help restore comfort, improve durability, and enhance interior aesthetics. We serve residential, commercial, hospitality, and office environments throughout Abu Dhabi.",
  ...
  },
  {
    ...
    description: "AL MAWASIM DECOR & CURTAINS provides SPC flooring, laminate flooring, and LVT flooring solutions for residential and commercial interiors. Our flooring systems combine durability, comfort, and stylish finishes. We deliver professional installation services tailored to every project.",
  ...
  },
  {
    ...
    description: "Our wallpaper installation and wall finishing services add character, texture, and sophistication to interior spaces. We provide premium materials and professional workmanship for residential and commercial projects. Our solutions help create stylish and welcoming environments.",
  ...
  },
  {
    ...
    description: "We provide quality carpets and custom carpet solutions designed to improve comfort and interior aesthetics. Our products are suitable for homes, offices, hotels, and commercial environments. We offer a variety of styles, textures, and finishes.",
  ...
  },
  {
    ...
    description: "AL MAWASIM DECOR & CURTAINS provides custom cupboards and wardrobe solutions designed to maximize storage and improve organization. Our solutions combine practicality, durability, and attractive finishes. We help create organized and functional interiors.",
...
export default function ServicesClient({ initialData }: ServicesClientProps) {
  ...
  const heroData = {
    tagline: "Our Service",
    title: initialData?.title || "Our Services",
    description: "Premium Curtains, Blinds & Interior Solutions Abu Dhabi",
    backgroundImage: "/services/bg-services.webp",
  };

  // Extract CMS Services array, safely defaulting to empty array if not present
  const cmsServices = initialData?.servicesConfig?.mainServices || [];

  // Merge CMS data with the static fallback layout data to preserve the alternating UI
  const displayServices = fallbackServicesData.map((staticSection, index) => {
    const cmsEquivalent = cmsServices[index];
    
    // If no CMS data overrides this index, just return the static fallback
    if (!cmsEquivalent) return staticSection;

    return {
      ...staticSection,
      title: cmsEquivalent.title || staticSection.title,
      description: cmsEquivalent.description || staticSection.description,
      image: cmsEquivalent.image?.url || staticSection.image,
      // Map CMS subServices, using the static image as a fallback since the Payload schema only handles text/icons for subServices
      subCards: cmsEquivalent.subServices?.length > 0 
        ? cmsEquivalent.subServices.map((sub: any, subIdx: number) => ({
            title: sub.title || staticSection.subCards?.[subIdx]?.title || "",
            desc: sub.description || staticSection.subCards?.[subIdx]?.desc || "",
            img: staticSection.subCards?.[subIdx]?.img || "/services/placeholder.webp",
          }))
        : staticSection.subCards,
    };
  });
