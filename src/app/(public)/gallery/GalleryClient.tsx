...
export default function GalleryClient({ images }: { images: Project[] }) {
  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden w-full bg-[#fdfbf74e]">
      ...
      <div className="fixed top-0 left-0 w-full h-screen -z-10 bg-[linear-gradient(90deg,rgba(26,26,26,0.85)_0%,rgba(255,255,255,0)_100%)]">
        <Image 
          src="/gallery/bg-gallery.webp" // Keep your local hero background
          alt="Gallery Hero"
          fill
      ...
      <div className="relative z-20 bg-[#FDFBF7] rounded-t-[2.5rem] shadow-[0_-10px_40px_rgba(0,0,0,0.05)] pb-32">
        ...
        <section className="relative z-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              {images.length === 0 ? (
                <p className="col-span-full text-center text-gray-500 py-10">Check back soon for new gallery images!</p>
              ) : (
                images.map((img) => (
                  <motion.div 
                    key={img.id} variants={fadeInUp}
                    onClick={() => setSelectedImage({ src: img.imageUrl, alt: img.title })}
                  ...
                  >
                    <Image 
                      src={img.imageUrl} alt={img.title} fill 
      ...
      <AnimatePresence>
        {selectedImage && (
          ...
          >
            ...
            >
              <Image 
                src={selectedImage.src} alt={selectedImage.alt} fill 
