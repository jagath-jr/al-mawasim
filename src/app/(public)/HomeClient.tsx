...
// Define the expected structure of the CMS data
...
export default function HomeClient({ initialData }: HomeClientProps) {
  ...
  // ============================================
  const homeConfig = initialData?.homeConfig || {};
  
  // Hero Data
  const heroData = homeConfig.heroSlider?.[0] || {};
  const heroBg = heroData.image?.url || "/herobg-img.webp";
  const heroHeadline = heroData.headline || "Premium Curtains, Blinds, Flooring & Wallpaper Services in Abu Dhabi";
  const heroSubheadline = heroData.subheadline || "AL MAWASIM DECOR & CURTAINS provides custom curtains, motorized curtains, roller blinds, zebra blinds, vertical blinds, Venetian blinds, SPC flooring, laminate flooring, wallpaper installation, and sofa upholstery services with professional installation across Abu Dhabi, UAE.";
  
  // Statistics Data
  const statsData = homeConfig.statistics || {};
  const targetSatisfaction = statsData.satisfactionScore || 100;
  const satisfactionLabel = statsData.satisfactionLabel || "Customer Satisfaction";
  const targetProjects = statsData.projectsCompleted || 500;
  const projectsLabel = statsData.projectsLabel || "Projects Completed";
  ...
  const servicesData = homeConfig.servicesShowcase?.length > 0 
    ? homeConfig.servicesShowcase.map((s: any) => ({
        title: s.title,
        img: s.image?.url || '/home/home-curtains-drapes.webp',
        desc: s.description
      }))
      ...
      const st3 = ScrollTrigger.create({
        ...
        onEnter: () => {
          ...
          gsap.to(counterTarget, {
            ...
            onComplete: () => {
              if (satisfactionRef.current) satisfactionRef.current.innerText = targetSatisfaction.toString();
              if (projectsRef.current) projectsRef.current.innerText = targetProjects.toString();
            }
          });
        },
        once: true
  ...
  }, [targetProjects, targetSatisfaction]);

  // ============================================
  // STATIC DATA (No CMS Fields Mapped Yet)
  // ============================================
  const features = [
    {
  ...
  return (
    <div className="w-full flex flex-col font-sans">
      ...
      >
        <div 
          ref={heroBgRef}
          className="absolute -top-[15%] left-0 w-full h-[130%] bg-cover bg-center bg-no-repeat"
            ...
            <div className="grid grid-cols-2 gap-4 sm:gap-6 lg:gap-6">
              ...
              <div className="flex flex-col gap-4 sm:gap-6 lg:gap-6">
                ...
                >
                  ...
                  <p className="text-xs sm:text-sm text-white/90 font-medium">{projectsLabel}</p>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-4 sm:gap-6 lg:gap-6">
                ...
                >
                  ...
                  <p className="text-xs sm:text-sm text-[#334155] font-medium">{satisfactionLabel}</p>
                </div>
                <div 
