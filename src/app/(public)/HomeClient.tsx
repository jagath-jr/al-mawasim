...
export default function HomeClient({ homeSettings, contactSettings, logos, services }: any) {
  ...
  const [quoteStatus, setQuoteStatus] = useState<"idle" | "success" | "error" | "rate_limited">("idle");
  ...
  useEffect(() => {
    ...
    if (heroTextRef.current) {
      gsap.fromTo(heroTextRef.current.children, { opacity: 0, y: 30 }, { opacity: 1, y: 0, duration: 0.8, stagger: 0.2, ease: 'power3.out', delay: 0.2 });
    }
    gsap.fromTo(heroFormRef.current, { opacity: 0, x: 50 }, { opacity: 1, x: 0, duration: 0.8, ease: 'power3.out', delay: 0.6 });
    if (heroBgRef.current && heroSectionRef.current) {
      gsap.to(heroBgRef.current, { yPercent: 50, ease: 'none', scrollTrigger: { trigger: heroSectionRef.current, start: 'top top', end: 'bottom top', scrub: true }});
    }

    // --- SERVICES ANIMATIONS ---
    if (servicesSectionRef.current && servicesCardsRef.current.length > 0) {
      gsap.fromTo(servicesCardsRef.current, { opacity: 0, y: 60 }, { opacity: 1, y: 0, duration: 0.8, stagger: 0.2, ease: 'power3.out', scrollTrigger: { trigger: servicesSectionRef.current, start: 'top 75%', toggleActions: 'play none none none' }});
    }
    ...
    if (aboutSectionRef.current) {
      ...
      if (masonryBoxesRef.current.length > 0) {
        const st1 = ScrollTrigger.create({ trigger: aboutSectionRef.current, start: 'top 75%', onEnter: () => {
          gsap.fromTo(masonryBoxesRef.current, { opacity: 0, scale: 0.9, y: -40 }, { opacity: 1, scale: 1, y: 0, duration: 0.8, stagger: 0.15, ease: 'back.out(1.2)' });
        }, once: true });
        scrollTriggers.current.push(st1);
      }

      const st2 = ScrollTrigger.create({ trigger: aboutSectionRef.current, start: 'top 75%', onEnter: () => {
        gsap.fromTo(aboutRightTextRef.current, { opacity: 0, x: 50 }, { opacity: 1, x: 0, duration: 1, ease: 'power3.out' });
      }, once: true });
      scrollTriggers.current.push(st2);

      const st3 = ScrollTrigger.create({ trigger: aboutSectionRef.current, start: 'top 75%', onEnter: () => {
        const counterTarget = { sat: 0, proj: 0 };
        gsap.to(counterTarget, {
          sat: homeSettings.statsSatisfaction,
          proj: homeSettings.statsProjects,
          duration: 2, ease: 'power2.out',
          onUpdate: () => {
            if (satisfactionRef.current) satisfactionRef.current.innerText = Math.floor(counterTarget.sat).toString();
            if (projectsRef.current) projectsRef.current.innerText = Math.floor(counterTarget.proj).toString();
          },
          onComplete: () => {
            if (satisfactionRef.current) satisfactionRef.current.innerText = homeSettings.statsSatisfaction.toString();
            if (projectsRef.current) projectsRef.current.innerText = homeSettings.statsProjects.toString();
          }
        });
      }, once: true });
      ...
      if (bannerCardsRef.current.length > 0) {
        ...
        if (bannerTrigger) {
          const st4 = ScrollTrigger.create({ trigger: bannerTrigger, start: 'top 85%', onEnter: () => {
            gsap.fromTo(bannerCardsRef.current, { opacity: 0, x: -60 }, { opacity: 1, x: 0, duration: 0.8, stagger: 0.2, ease: 'power3.out' });
          }, once: true });
    ...
    if (whyChooseSectionRef.current) {
      gsap.fromTo(whyChooseHeadingRef.current, { opacity: 0, y: -30 }, { opacity: 1, y: 0, duration: 0.8, ease: 'power3.out', scrollTrigger: { trigger: whyChooseSectionRef.current, start: 'top 80%', toggleActions: 'play none none none' }});
      if (whyChooseCardsRef.current.length > 0) {
        gsap.fromTo(whyChooseCardsRef.current, { opacity: 0, y: 50 }, { opacity: 1, y: 0, duration: 0.8, stagger: 0.15, ease: 'power3.out', scrollTrigger: { trigger: whyChooseSectionRef.current, start: 'top 70%', toggleActions: 'play none none none' }});
      }
    }

    const refreshTimeout = setTimeout(() => { ScrollTrigger.refresh(); }, 100);
    return () => { clearTimeout(refreshTimeout); scrollTriggers.current.forEach(st => st.kill()); };
  }, [homeSettings]);

  const features = [
    { icon: <Award size={36} strokeWidth={1.5} />, title: "Premium Quality Materials", desc: "High-quality materials selected for durability and elegance." },
    { icon: <Wrench size={36} strokeWidth={1.5} />, title: "Professional Installation", desc: "Precise installation delivered by experienced professionals." },
    { icon: <Palette size={36} strokeWidth={1.5} />, title: "Customized Solutions", desc: "Tailored interior solutions designed to perfectly match your space and style." },
    { icon: <Clock size={36} strokeWidth={1.5} />, title: "Timely Project Delivery", desc: "Projects completed efficiently and on schedule." }
  ];
    ...
    <div className="w-full flex flex-col font-sans">
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-6 lg:px-12 grid lg:grid-cols-2 gap-12 items-center pt-24 pb-16">
          ...
          <div ref={heroFormRef} id="quote" className="bg-[#FDFBF7]/95 backdrop-blur-sm p-8 md:p-10 rounded-xl shadow-2xl max-w-md ml-auto w-full scroll-mt-24">
            ...
            <form onSubmit={handleQuoteSubmit} className="space-y-4">
              ...
              <input type="tel" name="phone" required placeholder="Phone" className="w-full px-4 py-3 rounded-md bg-white border border-[#EAE1D0] text-[#1A1A1A] focus:ring-2 focus:ring-[#C5A869] outline-none transition-shadow" />
              <select name="service" required defaultValue="" className="w-full px-4 py-3 rounded-md bg-white border border-[#EAE1D0] text-[#1A1A1A] focus:ring-2 focus:ring-[#C5A869] outline-none transition-shadow appearance-none">
              ...
              {/* Status Messages */}
              {quoteStatus === "success" && <p className="text-green-600 mt-2 font-medium text-sm text-center">Quote request sent! We'll contact you shortly.</p>}
              {quoteStatus === "error" && <p className="text-red-600 mt-2 font-medium text-sm text-center">Failed to send. Please try again.</p>}
              {quoteStatus === "rate_limited" && <p className="text-amber-600 mt-2 font-medium text-sm text-center">Sending too quickly. Please wait a minute.</p>}
            </form>
          </div>
        </div>
      </section>
      ...
      <section ref={aboutSectionRef} className="py-24 px-6 lg:px-12 bg-[#FDFBF7] overflow-hidden">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <div className="grid grid-cols-2 gap-4 sm:gap-6 lg:gap-6">
            ...
            <div className="flex flex-col gap-4 sm:gap-6 lg:gap-6">
              <div ref={(el) => { masonryBoxesRef.current[2] = el; }} className="bg-[#C5A869] h-32 sm:h-40 md:h-44 rounded-xl flex flex-col justify-center items-center shadow-sm p-2 sm:p-4 text-center">
          ...
          <div ref={aboutRightTextRef} className="lg:pl-8">
            <span className="inline-block py-2 px-5 bg-[#EAE1D0] text-[#1A1A1A] text-sm font-semibold rounded-full mb-6 tracking-wide">Our Specialize</span>
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-[#1A1A1A] mb-6 leading-[1.2] tracking-tight whitespace-pre-line">{homeSettings.aboutTitle}</h2>
            <p className="text-gray-600 leading-relaxed text-sm sm:text-base md:text-lg whitespace-pre-line">{homeSettings.aboutDescription}</p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#F5F0E6] py-12 sm:py-16 px-6 lg:px-12 contact-banner-trigger overflow-hidden">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
          ...
          <div ref={(el) => { bannerCardsRef.current[1] = el; }} className="bg-[#1A1A1A] text-white p-6 sm:p-8 rounded-xl flex items-center gap-5 shadow-lg">
            <div className="rounded-full border-2 border-[#C5A869] p-2.5"><Phone className="text-[#C5A869]" size={20} fill="currentColor" strokeWidth={0} /></div>
            <div>
              <p className="text-sm text-gray-400 mb-1">Emergency Service</p>
              <p className="font-semibold text-sm lg:text-base text-white">{contactSettings.phone1}</p>
              {contactSettings.phone2 && <p className="font-semibold text-sm lg:text-base text-white">{contactSettings.phone2}</p>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 sm:gap-8 lg:gap-10">
            {features.map((feature, idx) => (
              <div key={idx} ref={(el) => { whyChooseCardsRef.current[idx] = el; }} className="flex flex-col items-start text-left group">
                <div className="w-20 h-20 bg-[#C5A869] rounded-2xl flex items-center justify-center text-[#1A1A1A] mb-6 shadow-sm transition-transform duration-300 group-hover:-translate-y-2">{feature.icon}</div>
                <h3 className="text-lg sm:text-xl font-bold text-[#1A1A1A] mb-3">{feature.title}</h3>
